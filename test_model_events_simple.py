...
async def test_model_takes_events(game_id: str):
    ...
    try:
        print(f"\n{'='*70}")
        print(f"TEST: Verifying Model Processes Events")
        ...
        if events_added == 0:
            print(f"  ⚠ No new events added (may have been already processed)")
        ...
        if features_added == 0:
            print(f"  ✗ No features generated - feature_state may not be processing events")
            return
        ...
        for i in range(max_wait // 2):
            ...
            if pred_cache:
                print(f"  ✓ Prediction generated and cached:")
        ...
        if not prediction_generated:
            print(f"  ✗ No prediction in cache after waiting")
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"  ✓ API returned prediction:")
