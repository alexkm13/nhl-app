        ...
        try:
            ...
            with psycopg.connect(self.db_url) as conn:
                ...
                with conn.cursor() as cur:
                    ...
                    variants = {}
                    for row in results:
                        variant_name, model_id, count, avg_pred, stddev_pred, median_pred, min_pred, max_pred = row
                        variants[variant_name] = {
                            "model_id": model_id,
                            "prediction_count": count,
                            "avg_prediction": float(avg_pred) if avg_pred else 0.0,
                            "stddev_prediction": float(stddev_pred) if stddev_pred else 0.0,
                            "median_prediction": float(median_pred) if median_pred else 0.0,
                    ...
                    if len(variant_names) >= 2:
                        ...
                        comparison = {
                            ...
                            "avg_prediction_diff_pct": (avg_diff / v2["avg_prediction"] * 100) if v2["avg_prediction"] > 0 else 0,
            ...
            with psycopg.connect(self.db_url) as conn:
                ...
                with conn.cursor() as cur:
                    ...
                    df = pd.DataFrame(results, columns=[
                        "time_bucket", "variant_name", "prediction_count", "avg_prediction"
                    ])
...
def print_ab_test_report(report: Dict):
    ...
    for variant_name, metrics in report.get("variants", {}).items():
        ...
        print(f"  Range: [{metrics['min_prediction']:.4f}, {metrics['max_prediction']:.4f}]")
    ...
    if comparison:
        ...
        print(f"    Avg Prediction Difference: {comparison['avg_prediction_diff']:.4f} ({comparison['avg_prediction_diff_pct']:.2f}%)")
        print(f"    Sample Sizes: {comparison['sample_size_a']:,} vs {comparison['sample_size_b']:,}")
