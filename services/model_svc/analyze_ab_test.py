...
def main():
    ...
    parser.add_argument(
        "--start-time", type=str, help="Start time (ISO format or 'N days ago')"
    )
    parser.add_argument(
        "--end-time", type=str, help="End time (ISO format, default: now)"
    )
    parser.add_argument("--game-id", help="Filter by specific game ID")
    parser.add_argument(
        "--days", type=int, help="Number of days to analyze (default: 7)"
    )
    ...
    report = analyzer.get_comparison_report(
        start_time=start_time, end_time=end_time, game_id=args.game_id
    )
