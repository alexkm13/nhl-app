        ...
        if self.model_path:
            ...
            if os.path.exists(self.model_path):
                if WinProbabilityModel is None:
                    raise ImportError("Trained model class not available. Install training dependencies.")
            ...
            possible_paths = [
                ...
                os.path.join("/app", "experiments", f"experiment_{self.model_id.split('_')[1]}_{self.model_id.split('_')[2]}", "model.pkl"),
            ]
            ...
            possible_paths.extend([
                ...
                os.path.join(training_dir, "experiments", f"experiment_{self.model_id.split('_')[1]}_{self.model_id.split('_')[2]}", "model.pkl"),
            ])
                    ...
                    if self.model_id in registry:
                                ...
                                if model_file_path:
                                    ...
                                    if not os.path.isabs(model_file_path):
                                        ...
                                        if registry_dir.startswith("/app"):
                                            ...
                                            model_file_path = os.path.join("/app", model_file_path)
                                        else:
                                            # Local: experiments/... -> training_dir/experiments/...
                                            model_file_path = os.path.join(training_dir, model_file_path)
                        ...
                        if model_dir:
                            ...
                            if model_dir.startswith('models/'):
                                if registry_dir.startswith("/app"):
                                    model_path = os.path.join("/app", model_dir, "model.pkl")
                                else:
                                    model_path = os.path.join(training_dir, model_dir, "model.pkl")
                            else:
                                model_path = os.path.join(registry_dir, model_dir, "model.pkl")
            ...
            if model_file:
                if WinProbabilityModel is None:
                    raise ImportError("Trained model class not available. Install training dependencies (lightgbm, pandas).")
                self.model = WinProbabilityModel.load(model_file)
                self.model_type = "trained"
            else:
                raise FileNotFoundError(f"Model {self.model_id} not found. Tried paths: {possible_paths}")
        else:
    ...
    def predict(self, features: dict) -> float:
        ...
        if self.model_type == "baseline":
            ...
            try:
                ...
                raise ImportError("pandas required for trained models. Install training dependencies.")
