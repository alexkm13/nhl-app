...
async def test_fetch_team_standings_success(mock_httpx_client, mock_redis):
    ...
    expected_data = {
        "standings": [
            {"teamAbbrev": {"default": "BOS"}, "wins": 10, "losses": 5, "otLosses": 2},
            {"teamAbbrev": {"default": "TOR"}, "wins": 8, "losses": 7, "otLosses": 1},
        ]
    }
...
async def test_fetch_nhl_daily_schedule_success(mock_httpx_client):
    ...
    expected_data = {
        "date": date,
        "games": [{"id": "2024020589", "homeTeam": {"id": 1}, "awayTeam": {"id": 2}}],
    }
...
async def test_fetch_nhl_daily_schedule_no_date(mock_httpx_client):
    ...
    expected_data = {"date": "2024-01-15", "games": []}
