...
from event_helpers import (
    EVENT_TYPE_MAPPING, calculate_strength_from_situation, calculate_goal_distance,
    format_goal_description, format_shot_description, format_penalty_description,
    get_game_situation, parse_situation_code
)
...
async def get_playbyplay(game_id: str, limit: int = 30):
    ...
    try:
        ...
        for play in plays:
            ...
            if type_code in [516, 524]:  # Skip stoppage, game-end (but keep period-end 517)
                continue
            
            # Include period-end events (517) and period-start events (520) for display
            if type_code not in [502, 503, 504, 505, 506, 507, 508, 509, 535, 536, 517, 520]:
                continue
        ...
        for play in processed_plays:
            ...
            if not strength_from_api or mapped_type != "GOAL":
                strength = calculate_strength_from_situation(situation, team, home_skaters, away_skaters)
            ...
            if mapped_type == "PERIOD_END":
                ...
                distance = calculate_goal_distance(x_coord, y_coord, play, team, period1_baseline)
                ...
                event_desc = format_goal_description(shot_type, distance, game_situation, strength, assists)
        ...
        crucial_events = [e for e in events if e.get("event_type") in ["GOAL", "PENALTY", "PERIOD_END"]]
        
        if is_complete:
            all_events = crucial_events
        else:
            non_crucial_events = [e for e in events if e.get("event_type") not in ["GOAL", "PENALTY", "PERIOD_END"]]
