            ...
            if response.status_code == 200:
                ...
                for team_data in standings:
                    if isinstance(team_data, dict):
                        team_info = {
                            "abbreviation": team_data.get("teamAbbrev", {}).get(
                                "default", ""
                            ),
                            "name": team_data.get("teamName", {}).get("default", ""),
                            "place_name": team_data.get("placeName", {}).get(
                                "default", ""
                            ),
                            "common_name": team_data.get("commonName", {}).get(
                                "default", ""
                            ),
                ...
                standings_list.sort(
                    key=lambda x: (x["points"], x["wins"]), reverse=True
                )
                ...
                raise HTTPException(
                    ...
                    detail=f"Standings API error {response.status_code}",
                )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error fetching standings: {str(e)}"
        )
