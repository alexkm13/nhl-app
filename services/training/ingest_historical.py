...
DATABASE_URL = os.environ.get(
    "DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/gamecast"
)
    ...
    try:
        ...
        for play in sorted_plays:
            ...
            if len(situation_code) >= 4:
                away_skaters = (
                    int(situation_code[1]) if situation_code[1].isdigit() else 5
                )
                home_skaters = (
                    int(situation_code[3]) if situation_code[3].isdigit() else 5
                )
            ...
            if (
                ...
                or (play_ts - last_snapshot_time) >= snapshot_interval
            ):
                snapshots.append(
                    {
                        "ts": play_ts,
...
async def main():
    ...
    parser.add_argument(
        "--start-date", type=str, default="2022-10-01", help="Start date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--end-date", type=str, default="2024-10-01", help="End date (YYYY-MM-DD)"
    )
