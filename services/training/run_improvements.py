...
async def train_improved_model(config_path: str, save_model: bool = True):
    ...
    train_samples = loader.create_training_samples(train_data, await loader.load_game_outcomes(
        training_config['train_start_date'],
        training_config['train_end_date']
    ))
    test_samples = loader.create_training_samples(test_data, await loader.load_game_outcomes(
    ...
    print(f"Created {len(train_samples)} training samples, {len(test_samples)} test samples")
    ...
    exclude_cols = {'ts', 'game_id', 'label', 'home_score', 'away_score', 
                    'strength', 'last_event', 'last_player_id', 'empty_net'}
        ...
        for metric in overall.keys():
            if metric in prev_metrics:
                ...
                if metric in ['log_loss', 'brier_score', 'calibration_error']:
                    if diff < 0:
                        print(f"  {metric:20s}: {diff:.4f} ({abs(pct):.2f}% improvement)")
                    else:
