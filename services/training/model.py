...
class WinProbabilityModel:
    ...
    def _train_lightgbm(
        ...
        if X_val is not None and y_val is not None:
            ...
            self.model = lgb.train(
                params, train_data, num_boost_round=self.config.get("n_estimators", 200)
            )
    ...
    def load(cls, filepath: str) -> "WinProbabilityModel":
        ...
        instance = cls(model_type=model_data["model_type"], **model_data["config"])
...
def train_model(
    ...
    model = WinProbabilityModel(
        model_type=model_type, **config.get("hyperparameters", {})
    )
