...
class FeatureEngineer:
    ...
    def _add_momentum_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for game_id in df['game_id'].unique():
            ...
            game_df['score_change_5min'] = game_df['score_change_5min'].rolling(
            ...
            game_df['recent_goals'] = game_df['recent_goals'].rolling(
                window=int(window_size / 30),
                min_periods=1
            ).sum()
    ...
    def get_feature_columns(self, df: pd.DataFrame) -> List[str]:
        ...
        exclude_cols = {
            'ts', 'game_id', 'label', 'home_score', 'away_score',
