...
async def process_events():
    ...
    while True:
        resp = await r.xreadgroup(
            GROUP, CONSUMER, streams={STREAM_EVENTS: ">"}, count=10, block=1000
        )
        if not resp:
            continue
        ...
        for stream, messages in resp:
            for mid, fields in messages:
                try:
                    ...
                    print(
                        f"[feature_state] {mid} -> features for {game_id}: {features['home_score']}-{features['away_score']} ({features['strength']})"
                    )
                except Exception as e:
